...
# private ip regex : /(^10\..*$)|(^192\.168\..*$)|(^172\.(1[6-9]|2[0-9]|3[0-1])\..*$)|(^127\..*$)|(^::1$)|(^::$)/
...
def filter(event)
  ...
  if !clean_ip_addresses.empty?
    ...
    if number_of_ip_addresses == 1
      event.set("[#{@parent_field}][Ip_info][version]", version_ip_addresses[0])
      event.set("[#{@parent_field}][Ip_info][public]", ip_addresses_public[0])
      event.set("[#{@parent_field}][Ip_info][type]", ip_addresses_type[0])
      event.set("[#{@parent_field}][Ip_info][rfc]", ip_addresses_rfc[0])
